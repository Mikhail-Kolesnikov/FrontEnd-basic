//Написать функцию, которая будет принимать 2 аргумента. Первый аргумент - массив, где элементы это обьекты, описывающие неких людей. 
//У каждого обьекта обязательно есть свойство "name". Наших массив players как пример подходит. Второй аргумент - это строка. 
//Входящая строка должна состоять только из одной буквы.
//Функция должна вернуть массив из обьектов, отфильтрованных так, чтобы были только обьекты где имя начинается на букву, 
...
const playersAndWords = (players, letter) => {
    const listOfplayers = [];

    players.forEach(element => {
        if (element.name.startsWith(letter)) {
            listOfplayers.push(element);
...
console.log(playersAndWords(players, 'A'));
